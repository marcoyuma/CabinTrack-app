...
- ~~The admin panel repo itself~~ — **done, see "Admin panel implementation" below.**
...
## Admin panel implementation (2026-08-16)

All four decisions are now implemented in the admin panel repo. Decisions 3 and 4 needed no work
there: the columns never existed in that repo's rewritten stays form, so nothing had to be
removed.

**Decision 1 — slug.** `utils/deriveSlug.ts` derives the slug from `name`;
`services/resolveUniqueSlug.ts` resolves conflicts with one `like` query rather than probing
candidates one at a time. The form shows the derived slug as a disabled preview, and `slug` was
dropped from the create schema entirely — there is no longer an input to validate. Since this is
a check-then-insert it is not race-proof, so `createStay.ts` also translates Postgres `23505`
into a "save again" message; the unique constraint stays the real guarantee.

**Decision 2 — lat/lng.** `services/geocoding.ts` + `utils/placeZoom.ts` +
`components/StayLocationPicker.tsx`. Used in **both** the create and edit forms, so raw
coordinate typing is gone from the admin panel entirely. The edit form deliberately does not
accept the `location` suggestion — that field is already correct there, and silently rewriting it
from a geocoder result is the failure mode this decision exists to prevent.

> ⚠️ **The geocoder is Photon, not Nominatim — this decision's named choice was overturned.**
> Not on result quality, but on policy: the OSMF usage policy forbids the exact interaction this
> input needs — *"Auto-complete search […] you must not implement such a service on the client
> side using the API."* A Nominatim picker is therefore stuck on press-a-button-and-wait, which
> is what made the first version feel unusable.
>
> **The reasoning behind the original choice survives intact.** Nominatim was picked because it
> needs no API key, account, or billing, matching the customer site's CARTO tile stack. Photon
> (komoot) is also OSM-derived, also keyless, also free — and is purpose-built for
> search-as-you-type with typo tolerance. Nothing about the no-key/no-billing principle is
> traded away.
>
> This also retires the open `User-Agent` problem noted in the previous revision: a browser
> cannot set that header, and Photon does not ask for it.
>
> **Accepted risk:** Photon's public instance states *"please be fair — extensive usage will be
> throttled"* and does not guarantee availability. For a handful of staff adding a handful of
> villas that is acceptable. The escape hatch is self-hosting Photon (hours-to-days, Elasticsearch
> rather than PostGIS), not reverting to Nominatim — reverting would reintroduce the policy
> conflict.

**Two follow-on fixes that were the actual cause of the bad first impression**, worth recording
because neither is about which geocoder is used:

- **Zoom must follow the result's granularity.** The first version used one fixed zoom (18,
  building level) for every result, so searching a city showed whichever building sat at the
  city's centroid. Results now `fitBounds` to the geocoder's bounding box (capped at zoom 18 so a
  building's few-metre box doesn't zoom to maximum), falling back to a per-type zoom table in
  `utils/placeZoom.ts` when a result has no box.
- **Results are restricted to Indonesia** by bounding box, so local place names can't lose to
  lookalikes abroad.

**Pin interaction follows the Airbnb/Uber pattern**: the pin is a fixed overlay at the centre of
the map and staff drag the map beneath it, rather than hunting a small draggable marker. The map
also reverse-geocodes the settled position and shows the street it landed on, so the pin can be
confirmed by reading a name instead of comparing decimal degrees.
