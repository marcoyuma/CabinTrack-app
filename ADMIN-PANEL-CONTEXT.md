...
**14 kolom wajib** (tidak punya default, insert gagal tanpa ini):

`slug`, `name`, `location`, `price_per_night`, `capacity`, `beds`, `area`, `description`,
`bed_type_label`, `capacity_label`, `lat`, `lng`, `airport_code`, `airport_city`

**Punya default, boleh dilewat:** `discount` (`0`), `is_new` (`false`), `is_featured`
(`false`), `created_at` (`now()`), `id` (identity).

**Satu-satunya kolom nullable di seluruh tabel:** `bed_type_note`.
...
| ------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `slug`                          | `name` yang dihuruf-kecilkan, spasi jadi tanda hubung. Tanpa nama daerah, tanpa "the" | Tidak rusak, tapi jadi tidak konsisten dengan 4 villa lain                                                                                                                                                                        |
...
| `capacity_label`                | Teks bebas, tapi polanya `"{N} adults and {M} children"` atau `"{N} adults"`          | **Tidak dicek terhadap `capacity`.** Menulis "sleeps 8" padahal `capacity = 4` menampilkan janji palsu di halaman, sementara stepper tamu tetap mentok di 4. Sebaiknya admin panel memperingatkan kalau angkanya tidak masuk akal |
| `bed_type_label`                | Praktiknya cuma `"Superking"` atau `"King"`                                           | —                                                                                                                                                                                                                                 |
| `bed_type_note`                 | Potongan kalimat pendek, tanpa titik: `"Crib on request"`                             | Kalau kosong/NULL, barisnya tidak dirender sama sekali — itu memang perilaku yang benar                                                                                                                                           |
| `airport_code` / `airport_city` | Semua villa `DPS` / `Denpasar`. Tidak ada tabel lookup                                | Membentuk link pencarian Google Flights. Kode salah = link ke pencarian yang salah                                                                                                                                                |
| `lat` / `lng`                   | 6 angka desimal, Bali (lat ≈ −8.5…−8.9, lng ≈ 115.0…115.6)                            | Peta dirender di zoom 18 (level jalan) — meleset ratusan meter langsung kelihatan salah                                                                                                                                           |
...
6. `capacity_label` cocok dengan angka `capacity`.
